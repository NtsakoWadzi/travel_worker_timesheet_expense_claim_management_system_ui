...
The system enables travelling workers to submit timesheets and travel-related expense claims, and enables managers to review, approve, reject, calculate, and pay those claims. The implemented solution consists of an Angular frontend, a Spring Boot REST API, JWT authentication, Spring Security authorization, and a MySQL database.
...
- Role-based routing for employees and managers.
...
| Admin | Seeded administrative role. | Protected admin endpoint exists, but no Angular admin screen is implemented. |
...
| FR-004 | Route employees to timesheets. | Non-manager users are routed to `/timesheet` after login. |
...
- No frontend registration screen exists although `/registerNewUser` exists on the backend.
